...
async def listar_vacinas(db: Session = Depends(get_db)):
...
async def buscar_vacina(vacina_id: int, db: Session = Depends(get_db)):
...
async def cadastrar_vacina(vacina: VacinaCreate, db: Session = Depends(get_db)):
...
async def atualizar_vacina(vacina_id: int, vacina: VacinaUpdate, db: Session = Depends(get_db)):
...
async def deletar_vacina(vacina_id: int, db: Session = Depends(get_db)):
